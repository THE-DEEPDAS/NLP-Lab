...
if __name__ == "__main__":
    ...
    # Process first 100 nouns and write analysis to file
    output_file = "fal_output.txt"
    with open(brown_nouns_file, 'r', encoding='utf-8') as f_in, open(output_file, 'w', encoding='utf-8') as f_out:
        count = 0
        for line in f_in:
            noun = line.strip().lower()
            if not noun:
                continue
            result = fst.analyze_word(noun)
            f_out.write(f"{result}\n")
            count += 1
            if count >= 100:
                break
    print(f"Wrote analysis of first 100 nouns to {output_file}")
